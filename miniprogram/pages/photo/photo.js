Page({
  ...
  getIcon(aaa) {
    ...
    db.collection('v').get().then(res => {
      ...
      console.log(list)
      wx.cloud.downloadFile({
        fileID: list[5].url, // 文件 ID
        success: res => {
          console.log('aaaaaaaaaaaaaaaaaaa', aaa.tempFilePath)
          ctx.drawImage(aaa.tempFilePath, 0, 0, 250, 250)
          ctx.drawImage(res.tempFilePath, 150, 150, 50, 50)
          // 获取icon列表
          ctx.draw()
        },
      })
